...
p <- ggplot(mss_data, aes(x=Positions, y=Stoichiometry)) + geom_boxplot(fill='white',color=colors()[190], lwd=0.75, notch=FALSE,fatten=0.0, width=0.5, outlier.shape=NA) 
dat <- ggplot_build(p)$data[[1]]
p <- p + geom_segment(data=dat, aes(x=xmin, xend=xmax, y=middle, yend=middle), colour=colors()[160], size=2.2)
p <- p + geom_jitter(width = 0.3, height=0, color=colors()[124],size=1.5,alpha=0.45)
...
p <- p + ylab("Substrate promiscuity (known)") + xlab("Substrate quality") + ggtitle('Known CK2 targets')
p <- p + theme(axis.title.y = element_text(vjust=-1.5))
p <- p+theme(axis.text.x=element_text(size=8.5),axis.text.y=element_text(size=11),axis.title.x=element_text(size=11.5,face="plain"),axis.title.y=element_text(size=10.5,face="plain"),plot.title=element_text(size=11,face='bold'))
...
p <- p + theme(axis.title.x = element_text(vjust=1.2))
p <- p+geom_text(data=data.frame(), aes(x=c(1:length(ck2_cov_list2)), y=rep(7,length(ck2_cov_list2)), label=rapply(ck2_cov_list2, function(x) length(x))), col='black', size=4.0)
