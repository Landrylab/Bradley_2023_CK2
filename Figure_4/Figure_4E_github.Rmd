...
p <- p+geom_text(x=7.20, y=0.12, label='n = 213',size=3.9,fontface='bold',col=colors()[617],check_overlap=T)
p <- p+geom_text(x=8.00, y=0.08, label='n = 44',size=3.9,fontface='bold',col="#E69F00",check_overlap = T)

# Ticks
p <- p+theme(axis.ticks.length=unit(-0.10, "cm"), axis.text.x = element_text(margin=unit(c(0.2,0.2,0.2,0.2), "cm")), axis.text.y = element_text(margin=unit(c(0.2,0.2,0.2,0.2), "cm")) )

p <- p+theme(axis.text=element_text(size=9.5),axis.title.x=element_text(size=12,face="plain"),axis.title.y=element_text(size=11,face="plain"))
p <- p + theme(legend.title=element_text(size=8,face='bold'),legend.text=element_text(size=8))
#p <- p+theme(legend.key.size = unit(1.0, 'cm'))
#p <- p+theme(legend.key.height = unit(0.40, 'inch'))
#p <- p+theme(legend.key.width = unit(0.40, 'inch'))
p <- p + guides(fill = guide_legend(keywidth = 0.375))
p <- p + guides(fill = guide_legend(keyheight = 0.9375))
p <- p+theme(legend.margin=margin(0,0,0,0),legend.box.margin=margin(-10,-5,-10,-10))
p <- p + theme(plot.margin = unit(c(-0.06,0.25,0.035,-0.5), "cm")) # t, r, b, l
p

g5 <- p

ggsave(file='Figure_4d.pdf', plot=p, width=6.5, height=5.0)
