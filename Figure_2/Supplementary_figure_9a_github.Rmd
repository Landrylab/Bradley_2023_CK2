...
p <- p+theme(axis.text=element_text(size=6),axis.title=element_text(size=9,face="plain"),plot.title=element_text(size=14,face='plain'))
...
p <- p+geom_text(data=data.frame(), aes(x=c(1:length(ck2_cov_list)), y=rep(8,length(ck2_cov_list)), label=rapply(ck2_cov_list, function(x) length(x))), col='black', size=2.5)
p

supp_fig_9a_mss_data <- mss_data
supp_fig_9a <- p

ggsave(file='Supplementary_figure_9a.pdf', plot=p, width=3.8, height=4.8)
