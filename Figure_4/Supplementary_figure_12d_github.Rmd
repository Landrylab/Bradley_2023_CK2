...
### Now iterate through each one of the phosphsosites and then out put the subtree with an ancestral S and all
...
# Identify ambiguously aligned phosphosites
...
for (i in 1:nrow(csnk2a_disorder_sites_dates_filter)) {
  ...
  ### Control for alignmet quality
  
  # Retrieve the filtered alignment
  
  filtered_al_seq <- seqinr::read.fasta(paste('vertebrate_ensembl_seq_anc_ST/',sequence_file,sep=''),seqtype='AA')
  ...
  ### Find candidate poorly aligned regions
  
  # Filter out if the phosphosite position has more than 20% gap sites
  
  flank <- lapply(getSequence(filtered_al_seq), function(x) x[(human_map_phospho_al)])
  gap_frac <- length(grep('-',flank))/length(flank)
  filter <- paste(sub_accession,'_',pos,sep='')
  if (gap_frac >= 0.2) {phospho_taxon_vec <- c(phospho_taxon_vec, 'NA'); filter_vec <- c(filter_vec, filter); next}
  
   # Filter if more than 20% of psites contain at least 1 S or T from -3 to +3
  
  flank <- lapply(getSequence(filtered_al_seq), function(x) x[c(((human_map_phospho_al-3):(human_map_phospho_al-1)),(human_map_phospho_al+1):(human_map_phospho_al+3))])
  S_frac <- length(unique(c(grep('S',flank),grep('S',flank))))/length(flank)
  if (S_frac >= 0.2) {phospho_taxon_vec <- c(phospho_taxon_vec, 'NA'); filter_vec <- c(filter_vec, filter); next}
  S_frac_vec <- c(S_frac_vec, S_frac)
...
## Well-aligned regions only

acc_pos <- paste(csnk2a_disorder_sites_dates_filter_r4s_long_sq_canon[,3],csnk2a_disorder_sites_dates_filter_r4s_long_sq_canon[,4],sep ='_')

# Filter out sites that map to ambiguously aligned regions
csnk2a_disorder_sites_dates_filter_r4s_long_sq_canon <- csnk2a_disorder_sites_dates_filter_r4s_long_sq_canon[!acc_pos %in% filter_vec,]
...
ggsave(file='Supplementary_figure_12d.pdf', plot=p, width=6.5, height=4.8)
