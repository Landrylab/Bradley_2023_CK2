...
### For this supplementary figure we will test if the results relating SQ to age and evolutionary rate are robust to uncertainties in the alignment
...
### Now we need to iterate through each one of the phosphsosites, and then estimate the timepoint at which the S emerged
...
for (i in 1:nrow(csnk2a_ksr_disorder_unique))  {  
  ...
  ############ Control for alignment quality
  
  # Filter out positions where more than 20% of sequences at the phosphoacceptor are represented by a gap
  
  flank <- lapply(getSequence(filtered_al_seq), function(x) x[(human_map_phospho_al)])
  gap_frac <- length(grep('-',flank))/length(flank)
  if (gap_frac >= 0.2) {phospho_taxon_vec <- c(phospho_taxon_vec, 'NA'); next}
  ...
  S_frac <- length(unique(c(grep('S',flank))))/length(flank)
  
  # Filter if more than 20% of psites contain at least 1 S from -3 to +3
  if (S_frac >= 0.2) {phospho_taxon_vec <- c(phospho_taxon_vec, 'NA'); next}
...
bin_count <- c(sum(mss_data[1:3,3]),sum(mss_data[4:6,3]),sum(mss_data[7:8,3]),sum(mss_data[9:11,3]),sum(mss_data[12:14,3]),sum(mss_data[15:17,3]),sum(mss_data[18:20,3]),sum(mss_data[21:23,3]))

bin_text <- c(rep('',2),bin_count[1],rep('',2),bin_count[2],rep('',1),bin_count[3],rep('',2),bin_count[4],rep('',2),bin_count[5],rep('',2),bin_count[6],rep('',2),bin_count[7],rep('',2),bin_count[8])
...
p <- p+geom_text(data=mss_data, aes(x=SQ),y=rep(1.025,23),label=bin_text,col='black', fontface='plain', size=3.6)

ggsave(file='Supplementary_figure_12c.pdf', plot=p, width=5.5, height=4.8)
