...
boxplot(olsen_ck2_sub[olsen_ck2_sub[,7] < 0.4,8],olsen_ck2_sub[olsen_ck2_sub[,7] > 0.4 & olsen_ck2_sub[,7] < 0.6,8],olsen_ck2_sub[olsen_ck2_sub[,7] > 0.6,8])
...
p <- ggplot(mss_data, aes(x=Positions, y=Stoichiometry)) + geom_boxplot(fill='white',color=colors()[160], lwd=1.1, notch=FALSE,fatten=0.0, outlier.shape=NA) 
dat <- ggplot_build(p)$data[[1]]
p <- p + geom_segment(data=dat, aes(x=xmin, xend=xmax, y=middle, yend=middle), colour=colors()[132], size=4)
p <- p + geom_jitter(width = 0.3, height=0, color='grey',size=1.5)
...
p <- p+theme(axis.text.x=element_text(size=6),axis.text.y=element_text(size=9),axis.title.x=element_text(size=8.5,face="plain"),axis.title.y=element_text(size=7.5,face="plain"),plot.title=element_text(size=18,face='plain'))
...
supp_fig_9b_mss_data <- mss_data
supp_fig_9b <- p

ggsave(file='Supplementary_figure_9B.pdf', plot=p, width=3.8, height=4.8)
...
sup_fig_9 <- plot_grid(supp_fig_9a,supp_fig_9b,ncol=2,rel_widths=c(1,1),labels=c('a','b'))

ggsave(file='Supp_fig_9.pdf', plot=sup_fig_9, width=7.4, height=3.1)
