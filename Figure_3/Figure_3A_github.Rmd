...
## Now model the specificity of CK2 by retrieving
...
# Scoring function for the calculation of substrate quality
...
# Extract flanking sequences of the high-quality phosphosites reported in Ochoa et al., 2019 
...
### Retrain the Ochoa prediction model without specificity features
...
# Train the model (about 15 minutes with 12 cores)
# Alter the number of cores as appropriate
...
# Functional scores for known CK2 sbstrates

```{r}

# For known CK2 substrates, extract the functional scores given in Ochoa et al., 2019 

library(seqinr)

ck2_sub <- paste(unlist(csnk2a_ksr_no_heira[,3]),unlist(csnk2a_ksr_no_heira[,4]),sep='_')
ST_scores_ck2 <- ST_scores[ST_scores[,1] %in% ck2_sub,]

ksr_df <- NULL
isoform_df <- NULL
missing_df <- NULL

for (i in 1:nrow(ST_scores_ck2)) {
  
  print(i)
  
  sub_id <- unlist(strsplit(as.character(ST_scores_ck2[i,1]),split='_'))[1]
  sub_pos <- unlist(strsplit(as.character(ST_scores_ck2[i,1]),split='_'))[2]
  ...
  ksr_seq <- c(sub_id,sub_pos,psite,ST_scores_ck2[i,2])
  ksr_df <- rbind(ksr_df,ksr_seq)
...
ck2_IDs <- unname(ksr_df[,1])
ck2_scores <- ksr_df[,4]

# Calculate the CK2 substrate quality for each one of these sites
...
# Plot the relationship between substrate quality and the phosphosite functional score (derived from Ochoa et al., 2019)
...
#ggplot graphical parameters

p <- p + ylab("Functional score") + xlab("Substrate quality") + ggtitle('')
p <- p + theme(axis.title.y = element_text(vjust=-1.5))
p <- p+theme(axis.text.x=element_text(size=6.0),axis.text.y=element_text(size=9),axis.title.x=element_text(size=10,face="plain"),axis.title.y=element_text(size=11,face="plain"),plot.title=element_text(size=18,face='bold'))
p <- p + theme(panel.border= element_blank())
p <- p+theme(plot.title = element_text(hjust = 0.5))
p <- p + theme(legend.position = "none") # 
p <- p + theme(plot.margin = unit(c(-0.35,0.1,0,0), "cm")) # t, r, b, l
p <- p + geom_hline(yintercept=med_ST_scores, linetype="dashed", color = "red", size=1)
p <- p + theme(axis.text.x = element_text(angle = 30, vjust = 1.25, hjust=1))
p <- p + theme(axis.title.x = element_text(vjust=5.0))
p <- p+geom_text(data=data.frame(), aes(x=c(1:8), y=rep(1.10,8)), label=c(unname(table(mss_data[,2]))),col=colors()[190], fontface='plain', size=3.0)
p

figure_3a <- p
mss_data_figure_3a <- mss_data

ggsave(file='Figure_2a.pdf', plot=p, width=6.1, height=4.8)
