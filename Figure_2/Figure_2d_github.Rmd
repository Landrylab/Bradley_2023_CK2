...
p <- p+theme(axis.text=element_text(size=8),axis.title=element_text(size=9,face="plain"),plot.title=element_text(size=14,face='plain'))
...
p <- p+geom_text(data=data.frame(), aes(x=c(1:length(ck2_cov_list)), y=rep(9,length(ck2_cov_list)), label=rapply(ck2_cov_list, function(x) length(x))), col='black', size=3.5)

figure_2d <- p
mss_data_figure_2d <- mss_data
